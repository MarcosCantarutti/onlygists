...
import { readOneAdapter, readAllAdapter, type ReadOneRow, type ReadAllRow } from './adapters'
...
export default (client: SupabaseClient<Database>) => ({
  ...
  async readAll({ username, from = 0, to = 10 }: ReadAllOptions) {
   ...
   async readOne(id: string){
    const response = await client.from('gists')
   ...
   // se for enviar o conteudo pro email
